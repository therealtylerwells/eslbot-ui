...
class Account extends Component<accountProps, accountState> {
  ...
  getMyJobs = async () => {
    const res = await fetch("https://api.eslbot.com/api/jobs?jobPosterId=" + localStorage.getItem("userId"));
  ...
  handleConfirm = () => {
    this.setState({ isConfirmOpen: false, loading: false });
    Axios.delete("https://api.eslbot.com/api/job?jobId=" + this.state.idToDelete)
      .then((response) => {
  ...
  handleRenew = (jobId: string) => {
    Axios.put("https://api.eslbot.com/api/renew?jobId=" + jobId).then((response) => {
      if (response.data.success) {
  ...
  handleEdit = async (jobId: string) => {
    ...
    const res = await fetch("https://api.eslbot.com/api/job?jobId=" + jobId);
  ...
  handleSaveEdit = (job: JobType) => {
    Axios.put("https://api.eslbot.com/api/job", job).then((response) => {
      if (response.data.success) {
    ...
    return (
      <div>
        ...
        <Menu>
          ...
          <Menu.Item name="manageAccount" active={activeItem === "manageAccount"} onClick={() => this.setState({ activeItem: "manageAccount" })}>
            Manage Account
          </Menu.Item>
        </Menu>
        {this.state.activeItem === "myJobs" ? (
          <div>
            <MyJobs handleDelete={this.handleDelete} handleRenew={this.handleRenew} handleEdit={this.handleEdit} jobs={this.state.userJobs} />
            <Confirm
